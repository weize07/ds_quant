...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .headline-reasoning { margin-top: -6px; margin-bottom: 16px; }
  .tab-nav { display: flex; gap: 8px; margin-bottom: 14px; }
  .tab-btn { border: 1px solid #d1d5db; background: #fff; color: #374151; border-radius: 8px; padding: 6px 12px; font-size: 13px; cursor: pointer; }
  .tab-btn.active { background: #eef2ff; border-color: #6366f1; color: #3730a3; font-weight: 600; }
  .tab-panel { display: none; }
  .tab-panel.active { display: block; }
  .info-note { font-size: 12px; color: #6b7280; margin-bottom: 8px; }
</style>
</head>
<body>
...
<div class="tab-nav">
  <button type="button" class="tab-btn active" data-tab="model">行情与模型</button>
  <button type="button" class="tab-btn" data-tab="info">信息源 / 资讯</button>
</div>
...
<details class="reasoning headline-reasoning">
  ...
  <p>当前 ${kpi.reversalProb != null ? kpi.reversalProb.toFixed(1) + '%' : '数值'} 被标记为“${kpi.reversalSignal || '待 monitor.js 生成'}”，表示可见因子已提供一定偏多的反转线索，但尚不足以替代价格确认。应继续观察郑糖是否突破确认位 ${CONFIG.sugarConfirm}；若跌破证伪位 ${CONFIG.sugarFalsify}，则反转假设被削弱，模型会限制该信号的上限。</p>
  <div class="formula">仪表盘模型假设：反转概率 ≈ 20%×ENSO气候 + 30%×郑糖/统计 + 15%×进口利润/内外价差 + 20%×股价/公司因子 + 15%×历史位置；再经确认位/证伪位门控。</div>
  <ul>
    <li>以上权重是仪表盘的模型解释/假设，并非分别由外部来源直接给出的预测权重。ENSO 维度观察 ONI 水平与暖化速度；郑糖/统计维度综合均线、动量、z-score 和历史分位。</li>
    <li>进口利润/内外价差维度用于衡量进口糖对内盘的供给压力：利润率低或倒挂较深时，进口冲击较弱，对内盘相对偏多。股价/公司因子则关注中粮糖业自身价格趋势和风险线；历史位置用于判断当前价格是否仍处在具备均值回归或上行赔率的区间。</li>
    <li>分数越高，代表这些维度共同支持“反转/上行准备度”的程度越强，而非价格必然上涨。量价、成交量与持仓量只作为确认或背离提示，不伪装成固定加权项。</li>
    <li>实际使用时，应以价格、成交量和持仓量是否同步确认为准，并遵守确认位、证伪位及个股风险线；若因子改善但价格未确认，或价格跌破风险位，应将其视为需要复核或否定的信号。</li>
  </ul>
</details>
...
<div id="tab-info" class="tab-panel">
  <div class="card wide">
    ...
    <div class="reasoning" style="margin-top:0">
      本页是信息源跟踪与人工整理面板，用于记录“来源—因子—频率—影响维度”的映射关系，并归档近期关键信息。
      目前不是实时新闻流；若未接入自动化采集，条目状态均以“待人工更新/复核”为准。
    </div>
  </div>
  <div class="card wide"><h3>信息源列表</h3><div id="infoSources"></div></div>
  <div class="card wide"><h3>近期重要资讯</h3><div id="recentNews"></div></div>
</div>
...
function impactTag(impact) {
  if (impact === '偏多') return 'red';
  if (impact === '偏空') return 'blue';
...
function renderRecentNews() {
  const groups = RECENT_NEWS.reduce((m, row) => {
    if (!m[row.category]) m[row.category] = [];
    m[row.category].push(row);
    return m;
  }, {});
...
function resizeCharts() {
  for (const id of ['c1', 'c2', 'c3', 'c4', 'c5', 'c6', 'c8', 'c9', 'c10', 'c11', 'c13']) {
